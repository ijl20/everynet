...
public class ParseEverynet {
    ...
    ParseEverynet(String feed_type, String area_id, Log logger)
        {
    ...
    public JsonArray parse_array(String page)
        {
            ...
            logger.log(Constants.LOG_DEBUG, "ParseFeed.parse_array called with feed_type:"+feed_type);
            logger.log(Constants.LOG_DEBUG, page);

            // If the feed type is already a (string) json array, then just parse and return
            if (feed_type.equals("JSON_ARRAY"))
                {
            ...
            // if this feed is already a json object, then embed in a single-element array and return
            if (feed_type.equals("JSON_OBJECT"))
                {
                   logger.log(Constants.LOG_DEBUG, "ParseFeed.parse_array parsing JSON_OBJECT");
            ...
            for (int i=0; i<record_templates.get(feed_type).size(); i++)
                {
                    ...
                    for (int j=0; j<record_template.fields.size(); j++)
                    {
                        ...
                        if (field_template.field_type.equals("fixed_int"))
                        {
                        ...
                        else if (field_template.field_type.equals("conditional_fixed_int"))
                        {
                        ...
                        else if (field_template.field_type.equals("fixed_string"))
                        {
                        ...
                        else if (field_template.field_type.equals("calc_minus"))
                        {
                            try {
                        ...
                        else if (field_template.field_type.equals("calc_plus"))
                        {
                            try {
                            int v1 = json_record.getInteger(field_template.s1);
                            int v2 = json_record.getInteger(field_template.s2);
                        ...
                        if (field_template.field_type.equals("int"))
                        {
                            try {
                        ...
                        else if (field_template.field_type.equals("string"))
                        {

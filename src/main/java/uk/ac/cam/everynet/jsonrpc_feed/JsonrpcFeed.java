...
public class JsonrpcFeed extends AbstractVerticle {
  ...
  public void start(Future<Void> fut) throws Exception {
    ...
    private void add_get_handler(Router router, String uri)
    {
        router.route(HttpMethod.GET,uri).handler( ctx -> {
                ...
                response.end("<h1>"+MODULE_NAME+" at "+uri+"</h1><p>Version "+VERSION+"</p>\n");
            });
    }
        ...
        for (int i=0; i<START_FEEDS.size(); i++)
            {
                ...
                // http.get and http.post bools tell whether to GET or receive POSTS
